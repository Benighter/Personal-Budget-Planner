...
const SubcategoryForm: React.FC<SubcategoryFormProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (existingSubcategory && isCumulativeMode) {
      finalAmount = existingSubcategory.allocatedAmount + inputAmount;
    }

    // Check if exceeds available budget within parent category
    if (maxAllocatableAmount !== undefined && finalAmount > maxAllocatableAmount) {
      const shortfall = finalAmount - maxAllocatableAmount;

      // Show reallocation prompt
    ...
    <>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-grow">
                ...
                <p className="text-slate-300 text-sm leading-relaxed mb-4">
                  The allocation of {formatAmountForAlert(reallocationDetails.requestedAmount)} exceeds the available budget within '{parentCategoryName}' by {formatAmountForAlert(reallocationDetails.shortfall)}.
                  {'\n\n'}
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-all duration-200"
                  >
                    Reallocate Budget
                  </button>
                </div>
      ...
      >
        <label
          ...
          className="block text-sm font-semibold text-slate-300 mb-2 transition-colors group-focus-within:text-emerald-400"
        >
          {isCumulativeMode && existingSubcategory ? 'Amount to Add' : 'Allocated Amount'} ({selectedCurrency})
        </label>
        <div className="relative group">
          <input
            type="number"
        ...
        {existingSubcategory && (
          <div className="mt-3 p-3 bg-gradient-to-r from-sky-500/10 to-violet-500/10 rounded-lg border border-sky-500/20">
            <label className="flex items-center cursor-pointer group">
              ...
              <span className="ml-3 text-sm text-slate-300 group-hover:text-white transition-colors">
                Add to existing amount ({formatAmountForAlert(existingSubcategory.allocatedAmount)})
              </span>
            </label>
            {isCumulativeMode && allocatedAmount && (
              <p className="mt-2 text-xs text-sky-400 font-semibold">
                New total will be: {formatAmountForAlert(existingSubcategory.allocatedAmount + (parseFloat(allocatedAmount) || 0))}
              </p>
            )}
          </div>
        )}
